...
module.exports = async function (req, res, next) {
  ...
  const findStreetWithCreatorId = async function (userId) {
    ...
    return Street.findOne({
      where: { creator_id: user.id, namespacedId: namespacedId }
    })
  }

  const findStreetWithNamespacedId = async function (namespacedId) {
    return Street.findOne({
      where: { creator_id: null, namespacedId: namespacedId }
    })
  }
  ...
  const handleFindStreet = async function (street) {
    ...
    const title = `${streetName} - streetTUner - TU Wien, Institut für Verkehrswissenschaften, Forschungsbereich Verkehrsplanung und Verkehrstechnik`
